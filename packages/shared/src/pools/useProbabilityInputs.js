...
function buildDefaultMap(entityIds) {
  return Object.fromEntries(
    (entityIds ?? []).map((entityId) => [entityId, { market: null, model: null }])
  )
}
...
/**
 * Read shared probability_inputs rows for a game product.
 *
 * @param {object} opts
 * @param {string} opts.productKey
 * @param {string[]} opts.entityIds
 * @param {string} [opts.entityType='series']
 */
export function useProbabilityInputs({ productKey, entityIds = [], entityType = 'series' }) {
