...
function narrativeTeamName(team, nextGame, isTeam1) {
  const rawName = isTeam1 ? nextGame.team1 : nextGame.team2;
  if (!rawName || rawName === "TBD") return team;

  // For names like "Utah State Aggies", drop the mascot and keep the school.
...
export default function ReportsRootingView() {
  const { PLAYERS, GAMES, LEVERAGE_GAMES, BEST_PATH } = usePoolData();
  const { pool } = usePool();
  ...
  const rootingRows = useMemo(() => {
    ...
    for (const game of GAMES) {
      ...
      for (const team of [game.team1, game.team2]) {
        ...
        const noteName = narrativeTeamName(team, nextGame, isTeam1);
        ...
        rows.push({
          ...
          nextGameLabel: `${nextGame.team1} vs ${nextGame.team2}`,
          ...
          note:
            rootScore >= 0
              ? `${noteName} supports more of this bracket's live path than it threatens.`
              : `${noteName} is more likely to strengthen competing paths than this bracket's own route.`,
        });
      }
    }
